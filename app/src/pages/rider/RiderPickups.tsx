    ...
    <div className="space-y-6">
      ...
      {myPickups.length === 0 ? (
        ...
        <div className="space-y-4">
          {myPickups.map((d) => (
            <div key={d.id} className="bg-white border border-gray-200 p-4 sm:p-6">
              <div className="flex justify-between items-start mb-4 flex-wrap gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-medium break-words">Pickup · {formatTime(d.assignedAt)}</p>
                  <p className="font-serif text-base sm:text-lg mt-1 truncate">Order #{d.orderId}</p>
                </div>
                <StatusBadge status={d.status} type="delivery" />
              </div>
              <div className="space-y-1.5 text-sm">
                <p className="flex items-center gap-1.5 text-gray-500 text-xs break-words">Delivery #{d.id} · assigned {formatTime(d.assignedAt)}</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 mt-4">
                {d.status === 'assigned' && (
                  <button
                    onClick={() => { markStatus(Number(d.id), 'picked_up'); showToast('Item collected', 'success'); }}
                    className="px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] font-medium bg-black text-white hover:bg-gray-900 whitespace-nowrap"
                ...
                <button
                  ...
                  className="px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] border border-yellow-400 text-yellow-800 hover:bg-yellow-50 flex items-center justify-center gap-1 whitespace-nowrap"
                >
                  <AlertTriangle className="h-3.5 w-3.5" /> Report Issue
                </button>
                <button
                  onClick={() => navigate(`/rider/deliveries/${d.id}`)}
                  className="px-4 py-2.5 text-[10px] uppercase tracking-[0.15em] border border-gray-300 hover:border-black flex items-center justify-center gap-1 whitespace-nowrap"
                >
