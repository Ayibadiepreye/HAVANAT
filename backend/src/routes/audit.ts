...
auditRouter.get('/', requireAuth, requireRole('admin', 'moderator'), async (req, res) => {
  ...
  const rows = await db.select().from(auditLog).where(where as any).orderBy(desc(auditLog.timestamp)).limit(Number(limit)).offset(Number(offset));
  res.json({ items: rows });
});
...
auditRouter.get('/export.csv', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (to) filters.push(lte(auditLog.timestamp, new Date(to)));
  const where = filters.length > 0 ? and(...filters) : undefined;
  const rows = await db.select().from(auditLog).where(where as any).orderBy(desc(auditLog.timestamp));
...
auditRouter.get('/stats', requireAuth, requireRole('admin', 'moderator'), async (req, res) => {
  ...
  const actionsToday = await db.select({ count: sql<number>`count(*)::int` }).from(auditLog).where(gte(auditLog.timestamp, startOfDay));
