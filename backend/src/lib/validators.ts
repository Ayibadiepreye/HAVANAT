...
export const CreateProductSchema = z.object({
  ...
  category: z.enum(['suits', 'blazers', 'trousers', 'vests', 'outerwear', 'shirts', 'accessories']),
  sizes: z.array(z.string()).default([]),
  colors: z.array(z.string()).default([]),
  fit: z.enum(['oversized', 'tailored', 'classic', 'slim', 'Oversized', 'Tailored', 'Classic', 'Slim']).optional(),
  inStock: z.boolean().default(true),
  stock: z.number().int().min(0).default(0),
  tags: z.array(z.string()).default([]),
  details: z.union([z.string(), z.record(z.any())]).default(''),
  care: z.string().default('').optional(),
  sku: z.string().optional(),
  occasion: z.string().optional(),
  deliveryFee: z.number().positive().optional(),
  deluxeDiscount: z.number().min(0).max(1).optional(),
  eliteDiscount: z.number().min(0).max(1).optional(),
  lowStockThreshold: z.number().int().min(0).optional(),
  published: z.boolean().default(true),
  isSneakPeek: z.boolean().default(false),
});
